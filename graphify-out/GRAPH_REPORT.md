...
## Communities (343 total, 62 thin omitted)
...
### Community 1 - "Control"
Cohesion: 0.01
Nodes (74): Control, Any, Call IUIAutomationElement::GetRuntimeId.         Return List[int], a list of in, Property Element.         Return `ctypes.POINTER(IUIAutomationElement)`., Property ControlTypeName., Define how to search.         control: `Control` or its subclass.         dept, Refind the control every searchIntervalSeconds seconds in maxSearchSeconds secon, Gets the position of the center of the control.         ratioX: float. (+66 more)

### Community 2 - ".GetPattern"
Cohesion: 0.04
Nodes (31): ComboBoxControl, DataItemControl, ListControl, Get a pattern by patternId.         patternId: int, a value in class `PatternId, Return `ExpandCollapsePattern` if it supports the pattern else None(Conditional, Return `ScrollPattern` if it supports the pattern else None(Conditional support, Return `SelectionPattern` if it supports the pattern else None(Conditional suppo, Show combobox's popup menu and select a item by name.         itemName: str. (+23 more)

### Community 3 - "TreeNode"
Cohesion: 0.04
Nodes (27): TreeNode, AppBarControl, CustomControl, DataGridControl, GroupControl, HeaderControl, HeaderItemControl, HyperlinkControl (+19 more)

### Community 4 - "uia/core.py"
Cohesion: 0.03
Nodes (76): BringWindowToTop(), ClipboardFormat, _dpi_metadata(), DwmIsCompositionEnabled(), ExtendedProperty, _get_monitor_effective_dpi(), _get_system_dpi(), GetConsoleOriginalTitle() (+68 more)

### Community 5 - "desktop/service.py"
Cohesion: 0.06
Nodes (24): _escape_text_for_sendkeys(), Escape special characters so uia.SendKeys types them correctly., _snapshot_profile_enabled(), Resolve a Windows Known Folder GUID path to an absolute filesystem path., resolve_known_folder_guid_path(), Browser, DesktopState, Display (+16 more)

### Community 6 - "with_analytics"
Cohesion: 0.06
Nodes (36): FastMCP, Decorator to wrap tool functions with analytics tracking., with_analytics(), Clipboard tool — copy/paste clipboard operations., register(), Any, DisplayInventory tool - read-only display and DPI metadata., _rect_to_dict() (+28 more)

### Community 7 - "enums.py"
Cohesion: 0.06
Nodes (54): IntEnum, ActiveEnd, AnimationStyle, AnnotationType, AsyncContentLoadedState, AutomationElementMode, AutomationIdentifierType, BulletStyle (+46 more)

### Community 8 - "WatchDog"
Cohesion: 0.06
Nodes (23): COMError, _AutomationClient, from_com_error(), Convert a ctypes.COMError into a typed UIAException. Only call with COMError ins, FocusChangedEventHandler, PropertyChangedEventHandler, Bump parent's consecutive-failure counter and request a client rebuild     once, _record_failure() (+15 more)

### Community 9 - "__main__.py"
Cohesion: 0.06
Nodes (47): CompletedProcess, Context, enable_debug(), is_debug(), Enable debug mode by setting the WINDOWS_MCP_DEBUG environment variable., Return True if debug mode is enabled via the WINDOWS_MCP_DEBUG environment varia, is_loopback_host(), Return True if host refers only to a loopback interface. (+39 more)

### Community 10 - "Desktop"
Cohesion: 0.07
Nodes (12): Desktop, Image, Rect, Get installed apps. Tries Get-StartApps first, falls back to shortcut scanning., Scan Start Menu folders for .lnk shortcuts as a fallback for Get-StartApps., Give any node of the app and it will return True if the app is a browser, False, Find a window by fuzzy name match. Returns (window, error_msg).         If the, Check if an app with the given AppID exists in shell:AppsFolder. (+4 more)

### Community 11 - "tree/service.py"
Cohesion: 0.06
Nodes (24): CachedControlHelper, CacheRequestFactory, UIA Caching Utilities for Performance Optimization  This module provides utili, Build a cached version of a control.                  Args:             node:, Factory for creating optimized cache requests for different scenarios., Creates a cache request optimized for tree traversal.         Caches all common, Helper class for working with cached controls., _is_comtypes_variant_ord_typeerror() (+16 more)
...
Cohesion: 0.05
Nodes (41): ControlFromCursor(), ControlFromCursor2(), ControlFromHandle(), ControlFromPoint(), ControlFromPoint2(), ControlsAreSame(), EnumAndLogControl(), EnumAndLogControlAncestors() (+33 more)

### Community 14 - "vdm/core.py"
Cohesion: 0.07
Nodes (35): c_void_p, create_desktop(), create_hstring(), delete_hstring(), get_all_desktops(), get_current_desktop(), _get_manager(), get_window_desktop_id() (+27 more)
...
Cohesion: 0.06
Nodes (15): ButtonControl, EditControl, ListItemControl, MenuItemControl, Return `InvokePattern` if it supports the pattern else None(Conditional support, ToolTipControl, TreeItemControl, InvokePattern (+7 more)

### Community 19 - "TreeElementNode"
Cohesion: 0.10
Nodes (22): Process windows sequentially to avoid COM apartment threading deadlock., _action_for(), Center, _format_semantic_node(), _node_meta_str(), _prune_structural(), Remove structural nodes that ended up with no children. Returns True = keep., Fix child ordering: tree_traversal visits reversed(children) so we reverse back. (+14 more)
...
Cohesion: 0.05
Nodes (15): CalendarControl, CheckBoxControl, DocumentControl, Return `GridPattern` if it supports the pattern else None(Must support according, Return `TablePattern` if it supports the pattern else None(Must support accordin, Return True if set successfully, Return `TableItemPattern` if it supports the pattern else None(Must support acco, TableControl (+7 more)

### Community 22 - "browser-cdp.mjs"
Cohesion: 0.07
Nodes (28): ensureCdp(), port, report, root, siteQ, verify, port, ensureMailEdge() (+20 more)

### Community 23 - "TextRange"
Cohesion: 0.07
Nodes (22): Call IUIAutomationTextRange::Clone.         return `TextRange`, identical to th, Call IUIAutomationTextRange::Compare.         textRange: `TextRange`., Call IUIAutomationTextRange::CompareEndpoints.         srcEndPoint: int, a valu, Call IUIAutomationTextRange::ExpandToEnclosingUnit.         Normalize the text, Call IUIAutomationTextRange::FindAttribute.         textAttributeID: int, a val, Call IUIAutomationTextRange::FindText.         text: str,         backward: bo, Call IUIAutomationTextRange::GetChildren.         textAttributeId: int, a value, Call IUIAutomationTextRange::GetEnclosingElement.         Return `Control` subc (+14 more)

### Community 24 - ".CreateControlFromElement"
Cohesion: 0.06
Nodes (18): Get children with pre-cached properties.                  This is the most sig, Return `LegacyIAccessiblePattern` if it supports the pattern else None., Return `Control` subclass or None., Get a sibling control that matches the condition.         forward: bool, if Tru, Return List[Control], a list of `Control` subclasses., Create a concreate `Control` from a com type `IUIAutomationElement`.         el, Create a concreate `Control` from a control instance, copy it.         control:, Retrieves a new UI Automation element with an updated cache.         cacheReque (+10 more)

### Community 25 - "infrastructure/__init__.py"
Cohesion: 0.10
Nodes (34): discover_config_path(), _list_of_strings(), load_config(), Path, Configuration loading and merge utilities for windows-mcp., Serialize *cfg* to TOML at *path*, writing only non-default values., Find config path using precedence: explicit > ~/.windows-mcp/config.toml., Load and validate TOML config file. Returns defaults when path is None. (+26 more)

### Community 26 - "GetScreenSize"
Cohesion: 0.08
Nodes (38): Only work on Windows 8/8.1, if current window is Metro UI.         waitTime: fl, Click(), DragDrop(), GetScreenSize(), GetVirtualScreenSize(), MiddleClick(), MiddleDragDrop(), MiddlePressMouse() (+30 more)
...
Cohesion: 0.09
Nodes (29): port, report, root, siteQ, inspectUrls, port, report, root (+21 more)
...
Cohesion: 0.07
Nodes (12): MenuBarControl, Return `DockPattern` if it supports the pattern else None(Conditional support ac, Return `TransformPattern` if it supports the pattern else None(Must support acco, ToolBarControl, DockPattern, Property CanMove.         Call IUIAutomationTransformPattern::get_CurrentCanMov, Property CanResize.         Call IUIAutomationTransformPattern::get_CurrentCanR, Property CanRotate.         Call IUIAutomationTransformPattern::get_CurrentCanR (+4 more)
...
Cohesion: 0.09
Nodes (5): BoundingBox, Any, TestBoundingBoxFromLocation, TestTraversalResult, TestBoundingBox
...
Cohesion: 0.09
Nodes (22): unifiedTokenProfileTemplateUrl(), unifiedTokenTemplateUrl(), extractTokenFromText(), loadPagesDeployCredentials(), loadTokenFile(), PAGES_PERMISSION_KEYS, PAGES_TOKEN_FILE, pagesTokenTemplateUrl() (+14 more)
...
Cohesion: 0.09
Nodes (27): is_dead_element_error(), is_retryable_error(), UI Automation and COM error codes.      Source: Winerror.h, UIAutomationCoreAp, Check if error code indicates element or window no longer exists., Check if error code indicates app is busy but alive — may succeed on retry., UIAError, UIA exception hierarchy for typed error handling., Element or window no longer exists — retrying will not help.      Covers: UIA_ (+19 more)
...
Cohesion: 0.09
Nodes (21): out, port, report, root, siteQ, summary, out, port (+13 more)
...
Cohesion: 0.09
Nodes (9): ProgressBarControl, Return `RangeValuePattern` if it supports the pattern else None(Conditional supp, ScrollBarControl, RangeValuePattern, Property IsReadOnly.         Call IUIAutomationRangeValuePattern::get_CurrentIs, Property LargeChange.         Call IUIAutomationRangeValuePattern::get_CurrentL, Property Maximum.         Call IUIAutomationRangeValuePattern::get_CurrentMaxim, Property SmallChange.         Call IUIAutomationRangeValuePattern::get_CurrentS (+1 more)

### Community 49 - "TestDisplayFiltering"
Cohesion: 0.10
Nodes (11): DisplayInfo, Display identity and geometry from the Windows monitor APIs., FakeDesktop, FakeMCP, MonkeyPatch, test_display_inventory_returns_display_dpi_metadata(), test_display_orientation_falls_back_to_bounds(), test_display_orientation_uses_current_mode() (+3 more)
...
Cohesion: 0.08
Nodes (9): ItemContainerPattern, ObjectModelPattern, Call IUIAutomationSpreadsheetPattern::GetItemByName.         name: str., Property TextContainer.         Call IUIAutomationSelectionContainer::get_TextC, Refer https://docs.microsoft.com/en-us/windows/win32/api/uiautomationclient/nn-u, Call IUIAutomationItemContainerPattern::FindItemByProperty.         control: `C, SpreadsheetPattern, TextChildPattern (+1 more)
...
Cohesion: 0.11
Nodes (13): Get a native handle from self or ancestors until valid and call native `ShowWind, Call native `ShowWindow(SW.Show)`.         Return bool, True if succeed otherwi, Call native `ShowWindow(SW.Hide)`.         waitTime: float         Return bool, Determine whether current control is top level., Set top level window topmost.         isTopmost: bool.         waitTime: float, Set top level window maximize., Restore window to normal state.         Similar to SwitchToThisWindow., Move window to screen center. (+5 more)
...
Cohesion: 0.10
Nodes (14): POINTER, CreatePattern(), CustomNavigationPattern, DropTargetPattern, GetPatternIdInterface(), IUnknown, uiautomation for Python 3. Author: yinkaisheng Source: https://github.com/yink, Call IUIAutomationTextRange::GetAttributeValue.         textAttributeId: int, a (+6 more)
...
Cohesion: 0.17
Nodes (16): resolveAuth(), deployPages(), ensureDeployCredentials(), pagesDeployEnv(), root, runWrangler(), runWranglerQuiet(), main() (+8 more)
...
Cohesion: 0.12
Nodes (11): Call IUIAutomationElement::SetFocus.         Refer https://docs.microsoft.com/e, Move cursor to control's internal position, default to center.         x: int,, Move cursor to control's center.         Return Tuple[int, int], two ints tuple, x: int, if < 0, click self.BoundingRectangle.right + x, if not None, ignore rati, x: int, if < 0, middle click self.BoundingRectangle.right + x, if not None, igno, Make control have focus first, move cursor to the specified position and mouse w, Make control have focus first and type a key.         `self.SetFocus` may not w, Show Desktop by pressing win + d (+3 more)

### Community 66 - "LegacyIAccessiblePattern"
Cohesion: 0.11
Nodes (9): LegacyIAccessiblePattern, Property ChildId.         Call IUIAutomationLegacyIAccessiblePattern::get_Curre, Property DefaultAction.         Call IUIAutomationLegacyIAccessiblePattern::get, Property Description.         Call IUIAutomationLegacyIAccessiblePattern::get_C, Property Help.         Call IUIAutomationLegacyIAccessiblePattern::get_CurrentH, Property KeyboardShortcut.         Call IUIAutomationLegacyIAccessiblePattern::, Call IUIAutomationLegacyIAccessiblePattern::DoDefaultAction.         Perform th, Call IUIAutomationLegacyIAccessiblePattern::GetCurrentSelection.         Return (+1 more)
...
Cohesion: 0.15
Nodes (4): _DxcamBackend, DXGI-based capture via the *dxcam* library., Return ``(device_idx, output_idx, region)`` when one DXGI output contains the re, TestDxcamBackend
...
Cohesion: 0.17
Nodes (12): _build_crop_box(), capture(), _crop_screenshot(), _get_backend(), Image, Rect, Capture a screenshot.  Subclasses must override., Return a cached singleton instance for the given backend *name*. (+4 more)
...
Cohesion: 0.12
Nodes (9): Call IUIAutomationWindowPattern::Close.         Close the window.         wait, Property CanMaximize.         Call IUIAutomationWindowPattern::get_CurrentCanMa, Property IsModal.         Call IUIAutomationWindowPattern::get_CurrentIsModal., Property IsTopmost.         Call IUIAutomationWindowPattern::get_CurrentIsTopmo, Property WindowInteractionState.         Call IUIAutomationWindowPattern::get_C, Property WindowVisualState.         Call IUIAutomationWindowPattern::get_Curren, Call IUIAutomationWindowPattern::SetWindowVisualState.         Minimize, maximi, Call IUIAutomationWindowPattern::WaitForInputIdle.         Cause the calling co (+1 more)
...
Cohesion: 0.17
Nodes (11): clearLegacyPagesTokenAlias(), loadDevVars(), removeDevVar(), resolvePurgeAuth(), main(), purgeEverything(), runEnsurePurgeToken(), probes (+3 more)
...
Cohesion: 0.18
Nodes (10): RegistryType, delete_entry(), get_value(), list_key(), Read a registry value at *path* with the given *name*., Create or update a registry value, creating the key if it does not exist., Delete a registry value when *name* is provided, otherwise remove the entire key, List values and sub-keys under *path*. (+2 more)
...
### Community 135 - "DxcamOutput"
Cohesion: 0.17
Nodes (3): DxcamOutput, A subclass that inherits name/priority without overriding should not register., TestBackendRegistry
...
### Community 138 - "Rect"
Cohesion: 0.17
Nodes (6): Rect, Get the cached bounding rectangle., Property BoundingRectangle.         Call IUIAutomationElement::get_CurrentBound, Find `word` in the control's text (via `TextPattern`) and return its bounding bo, Return the bounding box of the word at screen point (x, y), via `TextPattern`., Return the bounding box of every word in the control's visible text (via `TextPa
...
Cohesion: 0.24
Nodes (6): get_screenshot_backend(), Read the preferred backend from the environment variable., _isolate_backend_instances(), Unit tests for the screenshot capture module.  Tests the ``capture()`` public, Ensure each test gets a fresh backend instance pool., TestGetScreenshotBackend
...
Cohesion: 0.24
Nodes (5): InitializeUIAutomationInCurrentThread(), Initialize UIAutomation in a new thread.     If you want to use functionalities, Uninitialize UIAutomation in a new thread after calling InitializeUIAutomationIn, UIAutomationInitializerInThread, UninitializeUIAutomationInCurrentThread()

### Community 155 - "MultipleViewPattern"
Cohesion: 0.20
Nodes (5): MultipleViewPattern, Property CurrentView.         Call IUIAutomationMultipleViewPattern::get_Curren, Call IUIAutomationMultipleViewPattern::GetCurrentSupportedViews, todo., Call IUIAutomationMultipleViewPattern::GetViewName.         view: int, the cont, Call IUIAutomationMultipleViewPattern::SetCurrentView.         Set the view of

### Community 156 - "SelectionPattern2"
Cohesion: 0.22
Nodes (4): Property CurrentSelectedItem.         Call IUIAutomationSelectionPattern2::get_, Property FirstSelectedItem.         Call IUIAutomationSelectionPattern2::get_Cu, Property ItemCount.         Call IUIAutomationSelectionPattern2::get_CurrentIte, SelectionPattern2
...
Cohesion: 0.22
Nodes (7): Set top level window active., IsIconic(), IsWindowVisible(), IsIconic from Win32.     Determine whether a native window is minimized.     h, IsWindowVisible from Win32.     handle: int, the handle of a native window., SetForegroundWindow from Win32.     handle: int, the handle of a native window., SetForegroundWindow()
...
Cohesion: 0.39
Nodes (7): _as_args(), _launch_executable(), Path, App tool — launch, resize, switch applications., register(), _resolve_cwd(), _resolve_executable()

### Community 186 - "SpreadsheetItemPattern"
Cohesion: 0.25
Nodes (4): Property Formula.         Call IUIAutomationSpreadsheetItemPattern::get_Current, Call IUIAutomationSelectionPattern::GetCurrentAnnotationObjects.         Return, Call IUIAutomationSelectionPattern::GetCurrentAnnotationTypes.         Return L, SpreadsheetItemPattern
...
Cohesion: 0.36
Nodes (5): FakeMCP, register_tools(), test_get_coordinates_from_labels_returns_bulk_coordinates(), test_multiedit_uses_bulk_coordinate_resolution(), test_multiselect_uses_bulk_coordinate_resolution()
...
Cohesion: 0.48
Nodes (6): analyze(), dominant_colors(), find_tesseract(), image_to_base64(), Path, resolve_path()
...
Cohesion: 0.40
Nodes (3): childEnv(), npmCommand, runNpm()
...
Cohesion: 0.33
Nodes (3): Call IUIAutomationSelectionItemPattern::Select.         Clear any selected item, Call IUIAutomationTextRange::Select.         Select the span of text that corre, Call IUIAutomationLegacyIAccessiblePattern::Select.         Perform a Microsoft

### Community 214 - ".SetValue"
Cohesion: 0.33
Nodes (3): Call IUIAutomationTransformPattern2::IUIAutomationValuePattern::SetValue., Call IUIAutomationLegacyIAccessiblePattern::SetValue.         Set the Microsoft, Call IUIAutomationRangeValuePattern::SetValue.         Set the value of the con

### Community 215 - "SynchronizedInputPattern"
Cohesion: 0.33
Nodes (3): Call IUIAutomationSynchronizedInputPattern::Cancel.         Cause the Microsoft, Call IUIAutomationSynchronizedInputPattern::StartListening.         Cause the M, SynchronizedInputPattern
...
Cohesion: 0.30
Nodes (5): ControlType, PatternId, PropertyId, PatternId from IUIAutomation.     Refer https://docs.microsoft.com/en-us/window, ControlType from IUIAutomation.     Refer https://docs.microsoft.com/en-us/wind
...
### Community 256 - "SetWindowPos"
Cohesion: 0.50
Nodes (4): SetWindowPos from Win32.     handle: int, the handle of a native window.     h, handle: int, the handle of a native window.     isTopmost: bool     Return boo, SetWindowPos(), SetWindowTopmost()
...
## Knowledge Gaps
- **1027 isolated node(s):** `cloud-agent-install.sh script`, `NVM_DIR`, `payload`, `command`, `filePath` (+1022 more)
...
- **Why does `Control` connect `Control` to `.GetPattern`, `TreeNode`, `Desktop`, `tree/service.py`, `Rect`, `controls.py`, `AnnotationPattern`, `TransformPattern2`, `Return `InvokePattern` if it supports the pattern else None(Conditional support`, `GridPattern`, `TextRange`, `.CreateControlFromElement`, `GetScreenSize`, `MultipleViewPattern`, `SelectionPattern2`, `TransformPattern`, `DragPattern`, `RangeValuePattern`, `Refer https://docs.microsoft.com/en-us/windows/win32/api/uiautomationclient/nn-u`, `SpreadsheetItemPattern`, `TopLevel`, `patterns.py`, `.MoveCursorToInnerPos`, `LegacyIAccessiblePattern`, `WindowPattern`, `SynchronizedInputPattern`, `GridItemPattern`, `StylesPattern`?**
  _High betweenness centrality (0.112) - this node is a cross-community bridge._
- **Why does `Desktop` connect `Desktop` to `BoundingBox`, `desktop/service.py`, `__main__.py`, `IPAllowlistMiddleware`, `.execute_command`, `tree/service.py`, `OptionsMiddleware`, `test_deterministic_drag_desktop.py`, `TestDisplayFiltering`, `TreeElementNode`, `snapshot.py`, `test_multi_tools.py`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `WatchDog` connect `WatchDog` to `__main__.py`, `enums.py`?**
