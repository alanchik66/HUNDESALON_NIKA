...
В production bundle автоматически подключается:

- Live chat SendPulse:
  `<script src="https://cdn.pulse.is/livechat/loader.js" data-live-chat-id="6a89e797b7f95e2b6c0cf199" async></script>`

Отдельный SendPulse Chats Popup не загружается: он создаёт второй launcher и дублирует основной фирменный чат. Остаточный контейнер `sp-popups` также скрывается стилями; это не отключает серверную доставку личной консультации.
...
- Подробные условия услуг из `assets/js/price-catalog.js` добавляются для услуг, связанных через `priceRows[].key` с актуальным прайсом. Возраст и состав первого визита щенка берутся из того же источника, что и в модалке; цена и категории — только из текущего прайса. Старые цены и неиспользуемые варианты из вспомогательного каталога не импортируются.
...
- При изменении клиентских условий, состава услуг или цен нужно пересобрать знания, выполнить `npm run knowledge:check` и `npm run check:sendpulse-ai-knowledge`. Регрессионные тесты отклоняют устаревшую базу/индекс и проверяют подбор условий первого визита щенка на четырёх языках (`node --test functions/api/ai-chat.test.js`).
- Production-сборка обновляет знания автоматически, но локальное изменение ещё не меняет ответы опубликованного бота. После отдельно разрешённой публикации нужна проверка реальных ответов; ручная legacy-синхронизация SendPulse для основного бота не требуется.
- Endpoint выполняет локальный полнотекстовый поиск по компактным разделам, учитывает текущую локаль и передаёт модели не более 6 релевантных блоков общим объёмом до 10 500 символов. Это исключает повторную отправку всего документа и платные вызовы SendPulse File Search.
...
- Единственная разрешённая модель: `gpt-5.6-luna`; Responses API; reasoning `low`; максимальный ответ 500 токенов; ответы не сохраняются через API. Выбор модели зафиксирован на сервере, поэтому переменная окружения или запрос клиента не могут незаметно включить вторую модель.
- Стиль ответа: по умолчанию 1–3 коротких предложения, до 60 слов, только по вопросу клиента; подробности — по явной просьбе. Полная база остаётся внутренним справочником и не пересказывается блоками. Условия по возрасту, допустимому уходу и цене «от» сохраняются, если важны для ответа. Краткость задаётся инструкцией и `text.verbosity: low`, без механического обрезания предложений; фактическое соблюдение нужно проверять на ответах модели после публикации.
...
- Аватар внутри письма и аватар отправителя рядом с адресом — разные механизмы. HTML-шаблоны используют канонический `assets/images/brand/logo.png`; доменный аватар настраивается отдельно у почтовых провайдеров.
- Для межпровайдерного аватара используется BIMI. Не публиковать `default._bimi`, пока нет оригинального векторного знака SVG Tiny P/S и сертификата VMC/CMC, если его требует провайдер. Растровый `logo.png` нельзя встраивать в SVG: это не даст требуемого качества и не соответствует BIMI.
- Self-asserted BIMI не гарантирует показ во всех клиентах. Для Gmail и ряда других провайдеров требуется VMC или CMC; после получения сертификата его HTTPS URL добавляется в параметр `a=`.
- BIMI применяется только к письмам, которые проходят выравнивание DMARC; после настройки проверять реальные заголовки `Authentication-Results` у контрольного письма.
