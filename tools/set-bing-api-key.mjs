 ...
 * Bing Webmaster → Settings → API Access → Generate API Key.
 */
...
function looksLikeBingApiKey(value) {
  ...
  return /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i.test(key) || key.length >= 36;
}
...
if (!apiKey) {
  ...
  console.error('');
  console.error('Option A — automatic (Edge CDP, mail.ru account):');
  console.error('  npm run bing:edge');
  console.error('  npm run bing:fetch-api-key');
  console.error('');
  console.error('Option B — manual:');
  console.error('  Bing Webmaster → Settings → API Access → Generate API Key');
...
if (!looksLikeBingApiKey(apiKey)) {
  console.error('BING_WEBMASTER_API_KEY does not look like a Bing Webmaster API key (UUID-like hex).');
  process.exit(2);
}
