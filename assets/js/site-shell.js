...
<nav id="mobile-nav" aria-hidden="true">
  ...
  function enforceHeaderWeatherMenuPlacement(host) {
    ...
    try {
      ...
      // ─── Humidity chip: 3-column grid (label | ≈ | value) ───────────────────
      const syncHumidityApproxLayout = () => {
        const pressChip = root.querySelector(
          '.weather-header-card__chip[data-weather-metric="pressure"], .weather-header-card__chip--pressure-fallback'
        );
        const humChip = root.querySelector(
          '.weather-header-card__chip[data-weather-metric="humidity"], .weather-header-card__chip--humidity-fallback'
        );
        const pressLabel = root.querySelector('.weather-header-card__pressure-meta-label');

        if (!(humChip instanceof HTMLElement)) return;

        // ── 1. Sync font-size of humidity label to pressure label ──────────────
        const humLabelNode = humChip.children[0];
        if (humLabelNode instanceof HTMLElement && pressLabel instanceof HTMLElement) {
          const pressCS = window.getComputedStyle(pressLabel);
          humLabelNode.style.setProperty('font-size', pressCS.fontSize, 'important');
          humLabelNode.style.setProperty('line-height', pressCS.lineHeight, 'important');
          humLabelNode.style.setProperty('font-weight', pressCS.fontWeight, 'important');
          humLabelNode.style.setProperty('letter-spacing', pressCS.letterSpacing, 'important');
          humLabelNode.style.setProperty('text-transform', pressCS.textTransform, 'important');
          humLabelNode.style.setProperty('font-family', pressCS.fontFamily, 'important');
          humLabelNode.style.setProperty('display', 'block', 'important');
          humLabelNode.style.setProperty('white-space', 'nowrap', 'important');
          humLabelNode.style.setProperty('text-align', 'left', 'important');
          humLabelNode.style.setProperty('align-self', 'end', 'important');
          // strip plain textContent of stale ≈ chars
          const rawText = (humLabelNode.textContent || '').replace(/[≈~]/g, '').trim();
          if (humLabelNode.childElementCount === 0) {
            humLabelNode.textContent = rawText;
          }
        }

        // ── 2. Get/create ≈ node as direct chip child (col 2 of 3) ────────────
        let approxNode = humChip.querySelector(':scope > .weather-header-card__humidity-approx');
        const humValueNode = humChip.children[humChip.children.length - 1];
        if (!approxNode && humValueNode instanceof HTMLElement) {
          approxNode = document.createElement('span');
          approxNode.className = 'weather-header-card__humidity-approx';
          humChip.insertBefore(approxNode, humValueNode);
        }
        if (approxNode instanceof HTMLElement) {
          // size ≈ like the numeric value (bigger than label)
          const humValueCS = humValueNode instanceof HTMLElement ? window.getComputedStyle(humValueNode) : null;
          const approxSizePx = humValueCS ? Math.max(6, Number.parseFloat(humValueCS.fontSize) * 0.85 || 8) : 8;
          approxNode.textContent = '≈';
          approxNode.style.setProperty('font-size', `${approxSizePx.toFixed(1)}px`, 'important');
          approxNode.style.setProperty('line-height', '1', 'important');
          approxNode.style.setProperty('font-weight', '300', 'important');
          approxNode.style.setProperty('letter-spacing', '0', 'important');
          approxNode.style.setProperty('display', 'inline-block', 'important');
          approxNode.style.setProperty('white-space', 'nowrap', 'important');
          approxNode.style.setProperty('align-self', 'center', 'important');
        }

        // ── 3. Reshape chip to 3-column grid: label | ≈ | value ──────────────
        humChip.style.setProperty('display', 'grid', 'important');
        humChip.style.setProperty('grid-template-columns', '1fr auto auto', 'important');
        humChip.style.setProperty('align-items', 'end', 'important');
        humChip.style.setProperty('column-gap', '2px', 'important');
        humChip.style.setProperty('row-gap', '0', 'important');
        humChip.style.setProperty('white-space', 'nowrap', 'important');
        humChip.style.setProperty('padding-left', '0', 'important');
        humChip.style.setProperty('padding-right', '0', 'important');

        // ── 4. Sync chip width to pressure chip ───────────────────────────────
        if (pressChip instanceof HTMLElement) {
          const pressRect = pressChip.getBoundingClientRect();
          if (pressRect.width > 0) {
            const w = `${Math.round(pressRect.width * 10) / 10}px`;
            humChip.style.setProperty('width', w, 'important');
            humChip.style.setProperty('min-width', w, 'important');
            humChip.style.setProperty('max-width', w, 'important');
          }
          // left-edge alignment
          const pressLabelRect = pressLabel instanceof HTMLElement ? pressLabel.getBoundingClientRect() : null;
          const humLabelRect = humLabelNode instanceof HTMLElement ? humLabelNode.getBoundingClientRect() : null;
          if (pressLabelRect && humLabelRect && Math.abs(pressLabelRect.left - humLabelRect.left) > 0.5) {
            // grid 1fr naturally aligns left — no manual offset needed
          }
        }
      };
      runHeaderWeatherPostLayoutPass(syncHumidityApproxLayout, 2);
      runHeaderWeatherPostLayoutPass(syncHumidityApproxLayout, 3);
      // ─────────────────────────────────────────────────────────────────────────
