...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initSendmailForms = () => {
        ...
        forms.forEach((form) => {
            ...
            const isFeedbackForm = !isBookingForm && (
                form.closest('.complaint-form') !== null ||
                form.querySelector('select[name="subject"]') !== null
            );
    ...
    const initBookingModal = () => {
        ...
        if (!form || !serviceList || !calendarContainer || !timeSlotsContainer || !selectedServiceField || !selectedDateField || !selectedTimeField) {
            return;
        }
        ...
        const renderServiceList = () => {
            ...
            const services = bookingCopy.services.includes(state.selectedService) || !state.selectedService
        ...
        const moveToDateStep = () => {
            if (!state.selectedService) {
                window.alert(bookingCopy.chooseService);
                return;
            }
        ...
        const moveToContactStep = () => {
            if (!state.selectedDate) {
                window.alert(bookingCopy.chooseDate);
                return;
            }

            if (!state.selectedTime) {
                window.alert(bookingCopy.chooseTime);
                return;
            }
