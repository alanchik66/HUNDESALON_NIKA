...
(function () {
    ...
    function showTip(text, target) {
        ...
        tip.classList.remove('nika-tooltip--visible', 'nika-tooltip--below',
                             'nika-tooltip--left',    'nika-tooltip--right');
        tip.style.cssText = '';
        ...
        if (isBurger || inMobileNav) {
            ...
            if (preferRight && spaceRight >= needW)       placement = 'right';
                ...
                ['top', spaceTop], ['bottom', spaceBottom],
        ...
        let finalLeft, finalTop, arrowOffset = 0;
        ...
        if (placement === 'top' || placement === 'bottom') {
            ...
            finalLeft = (placement === 'right')
                ? rect.right + tooltipGap
                : rect.left  - tw - tooltipGap;
        }
    ...
    function attach(el, text) {
        ...
        el.addEventListener('mouseleave', () => { hideTimer = setTimeout(hideTip, 120); });
    ...
    function scanAndAttach() {
        ...
        document.querySelectorAll('.social-music-item[data-music-service="spotify"]').forEach(el => attach(el, mediaTooltip.spotify));
        document.querySelectorAll('.social-music-item[data-music-service="apple"]').forEach(el => attach(el, mediaTooltip.appleMusic));
        ...
        /* изображения без title */
        document.querySelectorAll('img:not([data-nika-tooltip])').forEach(el => {
            if (!el.closest('.logo') && !el.title) {
                const alt = el.alt;
                if (alt && alt !== 'HUNDESALON_NIKA') attach(el, alt);
            }
        });
    }
