...
| `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`                                           | Google Cloud IAM service account                      | стабильный серверный доступ к Calendar и Sheets                                            |
| `GOOGLE_SERVICE_ACCOUNT_SUBJECT`                                                                               | Google Workspace Admin                                | опциональная domain-wide delegation для Google API                                         |
| `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, `GOOGLE_OAUTH_REFRESH_TOKEN`                           | Google Auth Platform → OAuth client → Desktop app     | основной путь для обычного Google-аккаунта: Calendar и Sheets через refresh token          |
...
| `MS_TENANT_ID`, `MS_CLIENT_ID`, `MS_REFRESH_TOKEN`, `ONEDRIVE_UPLOAD_FOLDER`                                   | Microsoft Entra / Graph / OneDrive                    | единственное хранилище фото записи, вложений и расшифровок AI-чата                         |
| `BOOKING_CONFIRM_SECRET`                                                                                       | криптографически случайный Cloudflare secret          | подпись служебной ссылки; иначе используется Telegram secret                               |
| `OPENAI_API_KEY`                                                                                               | OpenAI API project secret                             | единая AI-модель; порода по фото — только явно                                             |
| `SENDPULSE_FROM`, `CLIENT_EMAIL_FROM`                                                                          | SendPulse → Senders                                   | основной отправитель transactional email для клиентов                                      |
| `CHAT_DB`                                                                                                      | Cloudflare D1 binding `nika-db`                       | клиенты, сессии, сообщения, дедупликация и маршрутизация ответов Telegram                  |
...
| `GOOGLE_SHARE_EMAIL`                                                                                           | Google/Workspace аккаунты администраторов             | доступ к созданным Calendar и Sheets; можно указывать несколько адресов через запятую      |
...
Файловый маршрут разделён намеренно: Google Sheets хранит поля заявки и служебную ссылку на OneDrive, но не байты
вложения. Фото из формы, любые вложения AI-чата и JSON-расшифровки хранятся только в OneDrive. Новая заявка получает
статус `pending`; событие Google Calendar создаётся только после нажатия администратором кнопки подтверждения в
служебном Telegram-чате либо после открытия служебной ссылки из письма и отдельного нажатия на странице подтверждения.
Обычный предпросмотр ссылки почтовой системой ничего не создаёт. Идентификатор события детерминирован по `request_id`,
поэтому повторное подтверждение не создаёт дубль.

После завершения загрузки вложения AI-чата OneDrive остаётся единственным постоянным файловым хранилищем. Бот потоково
передаёт оригинал размером до 50 МБ в служебный Telegram-чат методом `sendDocument`, поэтому изображение, видео или иной
файл доступен сотруднику без перекодирования. Для вложений больше 50 МБ Telegram получает только служебное описание и
ссылку OneDrive. Повторное завершение той же загрузки не создаёт второй файл или обычное повторное уведомление.

Проверенное состояние входящей почты на 30.08.2026: активное правило `info@hundesalon-nika.com` использует Worker `info-auto-reply`. Он сначала пересылает оригинал в существующий подтверждённый ящик, затем отправляет обычному отправителю информационный автоответ. Реальная доставка оригинала и автоответа проверена на административном ящике. `INFO_FORWARD_DESTINATION` должен указывать на конечный verified-ящик, а не возвращать письмо в `info@`.
...
Автоматические уведомления, отчёты доставки, рассылки и письма с собственных адресов пересылаются без автоответа. Исходящие `From`/`Return-Path` используют `noreply@`, а `Reply-To` — `support@`; оба обратных маршрута остаются прямой пересылкой без автоответчика. Не направляйте их обратно в info Worker. Проверенный SendPulse SMTP API не добавляет `Auto-Submitted` и не документирует произвольные заголовки: защита не должна зависеть от наличия этого поля в исходящем письме.
...
Скрипт создаёт Calendar и закрытую Google Sheets таблицу, предоставляет к ней доступ указанным Google/Workspace аккаунтам, ставит Cloudflare secrets для Production/Preview и не выводит OAuth secrets в консоль. Отдельную Google Drive папку для медиа он не создаёт.
